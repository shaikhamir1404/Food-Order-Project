...
from userException import customException
...
app.config['SQLALCHEMY_DATABASE_URI'] = "postgresql://postgres:root@127.0.0.1:5432/foodorder"
...
def signin():
    ...
    try:
        ...
        return render_template("products.html", products=allproduct, cart=session['cart_size'], msg=e, mob=session['mobile_no'])
...
def search():
    if request.method == "POST":
        ...
        try:
            ...
            return render_template("products.html", msg=e, cart=session['cart_size'], mob=session['mobile_no'])
...
def addToCart():
    ...
    try:
        ...
        if request.method == "POST":
            session['cart_items'].update({request.form.get('pid'): int(request.form.get('quantity'))})
...
@app.route("/Cart")
def cart():
    # Check routes is already logged in or not
    try:
        if session['mobile_no'] == '':
            raise customException("Need to login .!!!")
        cart_products = []
...
def purchase():
    try:
        ...
        entry = OrderProduct(order_date=datetime.date.today(), mobile_number=int(session['mobile_no']), address=address,
                             total_amount=int(total_amount), status='Pending')
        ...
        session['cart_size'] = 0
        allproduct = Product.query.all()
        return render_template("products.html", products=allproduct, mob=session['mobile_no'],
...
def previous_order():
    ...
    # Getting information of previous order of routes from database
...
def admin_login():
    ...
    if request.method == "POST":
        ...
        try:
            if idnumber == '':
                raise customException("Please Enter ID Number")
            elif password == '':
                raise customException("Please Enter Password")
            elif len(password) < 8 or len(password) > 15:
                raise customException("Length of Password Must be Between 8 to 15")
...
def administrator():
    ...
    query = "select * from order_product join order_details using(order_id) order by order_id desc"
    result = db.session.execute(query)
    return render_template("admin.html", result=result)


@app.route("/AdminChangeStatus", methods=['GET', 'POST'])
def admin_change_status():
    # Updating quantity by updating
    if request.method == "POST":
        status = request.form.get('status')
        order_id = request.form.get('oid')
        OrderProduct.query.filter_by(order_id=int(order_id)).update(dict({'status': status}))
        db.session.commit()
...
def add_product():
    ...
    if request.method == "POST":
        try:
            ...
            if not size.isalpha():
                raise customException("Size Should Contains Only Alphabets")
            if not productName.isalpha():
                raise customException("Product Name Should Contains Only Alphabets")
            if not rate.isdigit():
            ...
            result = Product(category=category, product_name=productName, size=size,
                             price=rate, image=image.filename)
            db.session.add(result)
            db.session.commit()
